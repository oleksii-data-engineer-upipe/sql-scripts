create MATERIALIZED VIEW billing_analyst.mv_summary_200
DISTKEY (site_id)
...
date_to_200 AS (
    SELECT external_id, date_1st_ph, MIN(CASE WHEN spend_nakop >= 200 THEN date_ph END) AS date_to_200
    FROM spend_accumulation
    GROUP BY 1, 2
)

SELECT 
    t11.date_to_200 AS date,
    t11.netw,
    t11.site_id,
    t11.os,
    t11.country,
    t11.age_group,
    COUNT(t11."200_day_to_day") AS "200_day_to_day",
    COUNT(t11."200_day_to_day_reg") AS "200_day_to_day_reg",
    COUNT(t11."200_nakop") AS "200_nakop",
    COUNT(t11."200_nakop_reg") AS "200_nakop_reg",
    COUNT(t11."200_delay_3_day") AS "200_delay_3_day",
    COUNT(t11."200_delay_3_day_reg") AS "200_delay_3_day_reg",
    COUNT(t11."200_day_to_day_women") AS "200_day_to_day_women",
    COUNT(t11."200_day_to_day_reg_women") AS "200_day_to_day_reg_women",
    COUNT(t11."200_nakop_women") AS "200_nakop_women",
    COUNT(t11."200_nakop_reg_women") AS "200_nakop_reg_women",
    COUNT(t11."200_delay_3_day_women") AS "200_delay_3_day_women",
    COUNT(t11."200_delay_3_day_reg_women") AS "200_delay_3_day_reg_women"
FROM (
    SELECT 
        t10.*,
        CASE WHEN t10.date_to_200 = t10.date_1st_ph AND t10.gender = 0 	THEN t10.external_id ELSE NULL
        END AS "200_day_to_day",
        CASE WHEN t10.date_to_200 = t10.date_reg AND t10.gender = 0 	THEN t10.external_id ELSE NULL
        END AS "200_day_to_day_reg",
        CASE WHEN t10.gender = 0 										THEN t10.external_id ELSE NULL
        END AS "200_nakop",
        CASE WHEN EXTRACT(MONTH FROM t10.date_to_200) = EXTRACT(MONTH FROM t10.date_reg) AND t10.gender = 0 THEN t10.external_id ELSE NULL
        END AS "200_nakop_reg",
        CASE WHEN t10.date_to_200 <= DATEADD(day, 3, t10.date_1st_ph) AND t10.gender = 0 	THEN t10.external_id ELSE NULL
        END AS "200_delay_3_day",
        CASE WHEN t10.date_to_200 <= DATEADD(day, 3, t10.date_reg) AND t10.gender = 0 		THEN t10.external_id ELSE NULL
        END AS "200_delay_3_day_reg",
        CASE WHEN t10.date_to_200 = t10.date_1st_ph AND t10.gender = 1 	THEN t10.external_id ELSE NULL
        END AS "200_day_to_day_women",
        CASE WHEN t10.date_to_200 = t10.date_reg AND t10.gender = 1 	THEN t10.external_id ELSE NULL
        END AS "200_day_to_day_reg_women",
        CASE WHEN t10.gender = 1 										THEN t10.external_id ELSE NULL
        END AS "200_nakop_women",
        CASE WHEN EXTRACT(MONTH FROM t10.date_to_200) = EXTRACT(MONTH FROM t10.date_reg) AND t10.gender = 1 THEN t10.external_id ELSE NULL
        END AS "200_nakop_reg_women",
        CASE WHEN t10.date_to_200 <= DATEADD(day, 3, t10.date_1st_ph) AND t10.gender = 1 	THEN t10.external_id ELSE NULL
        END AS "200_delay_3_day_women",
        CASE WHEN t10.date_to_200 <= DATEADD(day, 3, t10.date_reg) AND t10.gender = 1 		THEN t10.external_id ELSE NULL
        END AS "200_delay_3_day_reg_women"
    FROM (
		SELECT 
		    d2h.*,
		...
		WHERE d2h.date_to_200 IS NOT null
    ) t10
) t11
GROUP BY t11.date_to_200, t11.netw, t11.site_id, t11.os, t11.country, t11.age_group
;
