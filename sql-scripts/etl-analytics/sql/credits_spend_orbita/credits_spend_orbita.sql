...
BEGIN
    ...
    FROM redshift_analytics_db.prodmysqldatabase.log l
    WHERE l.action_type = 'REGISTRATION_BONUS'
    AND DATE(l.date_created) > DATEADD(month, -1, CURRENT_DATE);
    ...
    DELETE FROM prod_analytic_db.credits_spend_orbita.users_free_given WHERE date_created > DATEADD(month, -1, CURRENT_DATE);
    INSERT INTO prod_analytic_db.credits_spend_orbita.users_free_given
    SELECT * FROM temp_users_free_given;

    DELETE FROM prod_analytic_db.credits_spend_orbita.users_free_spent WHERE date_created > DATEADD(month, -1, CURRENT_DATE);
    INSERT INTO prod_analytic_db.credits_spend_orbita.users_free_spent
    SELECT * FROM temp_users_free_spent;

    DELETE FROM prod_analytic_db.credits_spend_orbita.man_paid_actions WHERE date > DATEADD(month, -1, CURRENT_DATE);
