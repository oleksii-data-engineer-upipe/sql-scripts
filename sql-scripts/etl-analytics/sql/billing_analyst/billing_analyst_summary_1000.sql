create MATERIALIZED VIEW billing_analyst.mv_summary_1000
DISTKEY (site_id)
...
date_to_1000 AS (
    SELECT external_id, date_1st_ph, MIN(CASE WHEN spend_nakop >= 1000 THEN date_ph END) AS date_to_1000
    FROM spend_accumulation
    GROUP BY 1, 2
)

SELECT 
    t11.date_to_1000 AS date,
    t11.netw,
    t11.site_id,
    t11.os,
    t11.country,
    t11.age_group,
    COUNT(t11."1000_day_to_day") AS "1000_day_to_day",
    COUNT(t11."1000_day_to_day_reg") AS "1000_day_to_day_reg",
    COUNT(t11."1000_nakop") AS "1000_nakop",
    COUNT(t11."1000_nakop_reg") AS "1000_nakop_reg",
    COUNT(t11."1000_delay_3_day") AS "1000_delay_3_day",
    COUNT(t11."1000_delay_3_day_reg") AS "1000_delay_3_day_reg",
    COUNT(t11."1000_day_to_day_women") AS "1000_day_to_day_women",
    COUNT(t11."1000_day_to_day_reg_women") AS "1000_day_to_day_reg_women",
    COUNT(t11."1000_nakop_women") AS "1000_nakop_women",
    COUNT(t11."1000_nakop_reg_women") AS "1000_nakop_reg_women",
    COUNT(t11."1000_delay_3_day_women") AS "1000_delay_3_day_women",
    COUNT(t11."1000_delay_3_day_reg_women") AS "1000_delay_3_day_reg_women"
FROM (
    SELECT 
        t10.*,
        CASE WHEN t10.date_to_1000 = t10.date_1st_ph AND t10.gender = 0 	THEN t10.external_id ELSE NULL
        END AS "1000_day_to_day",
        CASE WHEN t10.date_to_1000 = t10.date_reg AND t10.gender = 0 	THEN t10.external_id ELSE NULL
        END AS "1000_day_to_day_reg",
        CASE WHEN t10.gender = 0 										THEN t10.external_id ELSE NULL
        END AS "1000_nakop",
        CASE WHEN EXTRACT(MONTH FROM t10.date_to_1000) = EXTRACT(MONTH FROM t10.date_reg) AND t10.gender = 0 THEN t10.external_id ELSE NULL
        END AS "1000_nakop_reg",
        CASE WHEN t10.date_to_1000 <= DATEADD(day, 3, t10.date_1st_ph) AND t10.gender = 0 	THEN t10.external_id ELSE NULL
        END AS "1000_delay_3_day",
        CASE WHEN t10.date_to_1000 <= DATEADD(day, 3, t10.date_reg) AND t10.gender = 0 		THEN t10.external_id ELSE NULL
        END AS "1000_delay_3_day_reg",
        CASE WHEN t10.date_to_1000 = t10.date_1st_ph AND t10.gender = 1 	THEN t10.external_id ELSE NULL
        END AS "1000_day_to_day_women",
        CASE WHEN t10.date_to_1000 = t10.date_reg AND t10.gender = 1 	THEN t10.external_id ELSE NULL
        END AS "1000_day_to_day_reg_women",
        CASE WHEN t10.gender = 1 										THEN t10.external_id ELSE NULL
        END AS "1000_nakop_women",
        CASE WHEN EXTRACT(MONTH FROM t10.date_to_1000) = EXTRACT(MONTH FROM t10.date_reg) AND t10.gender = 1 THEN t10.external_id ELSE NULL
        END AS "1000_nakop_reg_women",
        CASE WHEN t10.date_to_1000 <= DATEADD(day, 3, t10.date_1st_ph) AND t10.gender = 1 	THEN t10.external_id ELSE NULL
        END AS "1000_delay_3_day_women",
        CASE WHEN t10.date_to_1000 <= DATEADD(day, 3, t10.date_reg) AND t10.gender = 1 		THEN t10.external_id ELSE NULL
        END AS "1000_delay_3_day_reg_women"
    FROM (
		SELECT 
		    d2h.*,
		...
		WHERE d2h.date_to_1000 IS NOT null
    ) t10
) t11
GROUP BY t11.date_to_1000, t11.netw, t11.site_id, t11.os, t11.country, t11.age_group
;
