CREATE MATERIALIZED VIEW prod_analytic_db.billing_analyst.mv_billing_stat
DISTKEY (external_id)
SORTKEY ("time")
AS
WITH base_payments AS (
    ...
    WHERE cp.reason NOT LIKE '%bandone%'
      ...
      AND cp.external_id IN (
          ...
          FROM redshift_analytics_db.prodmysqldatabase.v2_center_payment cp
          WHERE DATE(CONVERT_TIMEZONE('UTC', 'Europe/Kiev', cp.date_created::timestamp)) >= DATEADD(year, -1, CURRENT_DATE)
      )
),
...
sphera_events AS (
    ...
    WHERE e."type" IN ('PAYMENT_FAILED', 'PAYMENT_SUCCEDED')
      ...
      AND up.email NOT LIKE '%irens%'
      AND up.email NOT LIKE '%galaktica%'
      AND SPLIT_PART(e."details"."paymentResponse".message::varchar, 'Error:', 2) NOT SIMILAR TO 
          '%(bandone|Cancelled by customer|sufficient funds|AUTH Error \(code = 60022\)|— \(code = —\)|Stolen Card|Your card has expired\. \(code = expired_card\)|Expired Card|\(code = 60022\)|MAC 02: Policy)%'
),
...
enriched_data AS (
    SELECT t1.*,
           up.name::varchar||', '::varchar||up.age::varchar AS user,
