SELECT t11.`date_to_200` AS `date`,
    t11.`netw`,
    t11.`site_id`,
    t11.`os`,
    t11.`country`,
    COUNT(t11.`200_day_to_day`) `200_day_to_day`,
    COUNT(t11.`200_day_to_day_reg`) `200_day_to_day_reg`,
    COUNT(t11.`200_nakop`) `200_nakop`,
    COUNT(t11.`200_nakop_reg`) `200_nakop_reg`,
    COUNT(t11.`200_delay_3_day`) `200_delay_3_day`,
    COUNT(t11.`200_delay_3_day_reg`) `200_delay_3_day_reg`,
    COUNT(t11.`200_day_to_day_women`) `200_day_to_day_women`,
    COUNT(t11.`200_day_to_day_reg_women`) `200_day_to_day_reg_women`,
    COUNT(t11.`200_nakop_women`) `200_nakop_women`,
    COUNT(t11.`200_nakop_reg_women`) `200_nakop_reg_women`,
    COUNT(t11.`200_delay_3_day_women`) `200_delay_3_day_women`,
    COUNT(t11.`200_delay_3_day_reg_women`) `200_delay_3_day_reg_women`
from (
        SELECT t10.*,
            if(
                t10.`date_to_200` = t10.`date_1st_ph`
                AND t10.`gender` = 0,
                t10.`external_id`,
                NULL
            ) `200_day_to_day`,
            if(
                t10.`date_to_200` = t10.`date_reg`
                AND t10.`gender` = 0,
                t10.`external_id`,
                NULL
            ) `200_day_to_day_reg`,
            if(t10.`gender` = 0, t10.`external_id`, NULL) `200_nakop`,
            if(
                month(t10.`date_to_200`) = month(t10.`date_reg`)
                AND t10.`gender` = 0,
                t10.`external_id`,
                NULL
            ) `200_nakop_reg`,
            if(
                t10.`date_to_200` <= t10.`date_1st_ph` + INTERVAL 3 DAY
                AND t10.`gender` = 0,
                t10.`external_id`,
                NULL
            ) `200_delay_3_day`,
            if(
                t10.`date_to_200` <= t10.`date_reg` + INTERVAL 3 DAY
                AND t10.`gender` = 0,
                t10.`external_id`,
                NULL
            ) `200_delay_3_day_reg`,
            if(
                ...
                t10.`external_id`,
                NULL
            ) `200_day_to_day_women`,
            if(
                t10.`date_to_200` = t10.`date_reg`
                AND t10.`gender` = 1,
                t10.`external_id`,
                NULL
            ) `200_day_to_day_reg_women`,
            if(t10.`gender` = 1, t10.`external_id`, NULL) `200_nakop_women`,
            if(
                month(t10.`date_to_200`) = month(t10.`date_reg`)
                AND t10.`gender` = 1,
                t10.`external_id`,
            ...
            if(
                ...
                t10.`external_id`,
                NULL
            ) `200_delay_3_day_women`,
            if(
                t10.`date_to_200` <= t10.`date_reg` + INTERVAL 3 DAY
                AND t10.`gender` = 1,
                t10.`external_id`,
                NULL
            ) `200_delay_3_day_reg_women`
        from (
                SELECT t9.*,
                ...
                from (
                        SELECT t8.`external_id`,
                            t8.`date_1st_ph`,
                            min(t8.`date_to_200`) `date_to_200`
                        from (
                                SELECT t7.*,
                                    if(t7.`spend_nakop` >= 200, t7.`date_ph`, NULL) `date_to_200`
                                from (
                                        SELECT t6.*,
                                            SUM(t6.`spend`) OVER (
                        ...
                        WHERE t8.`date_to_200` IS NOT NULL
