CREATE MATERIALIZED VIEW prod_analytic_db.summary.mv_summary_1000
...
threshold_dates AS (
   SELECT 
       ...
       MIN(spend_date) AS date_to_1000
   FROM spend_data
...
SELECT 
   date_to_1000 AS date,
   netw,
   site_id,
   os,
   country,
   -- Стандартні метрики для чоловіків
   COUNT(CASE WHEN gender = 0 AND date_to_1000 = first_purchase_date 		THEN external_id END) AS "1000_day_to_day",
   COUNT(CASE WHEN gender = 0 AND date_to_1000 = reg_date 				THEN external_id END) AS "1000_day_to_day_reg",
   COUNT(CASE WHEN gender = 0  											THEN external_id END) AS "1000_nakop",
   COUNT(CASE WHEN gender = 0 AND DATE_PART(month, date_to_1000) = DATE_PART(month, reg_date) THEN external_id END) AS "1000_nakop_reg",
   COUNT(CASE WHEN gender = 0 AND date_to_1000 <= first_purchase_date + 3 THEN external_id END) AS "1000_delay_3_day",
   COUNT(CASE WHEN gender = 0 AND date_to_1000 <= reg_date + 3 			THEN external_id END) AS "1000_delay_3_day_reg",

   -- Ті ж метрики для жінок
   COUNT(CASE WHEN gender = 1 AND date_to_1000 = first_purchase_date 		THEN external_id END) AS "1000_day_to_day_women",
   COUNT(CASE WHEN gender = 1 AND date_to_1000 = reg_date 				THEN external_id END) AS "1000_day_to_day_reg_women",
   COUNT(CASE WHEN gender = 1 											THEN external_id END) AS "1000_nakop_women",
   COUNT(CASE WHEN gender = 1 AND DATE_PART(month, date_to_1000) = DATE_PART(month, reg_date) THEN external_id END) AS "1000_nakop_reg_women",
   COUNT(CASE WHEN gender = 1 AND date_to_1000 <= first_purchase_date + 3 THEN external_id END) AS "1000_delay_3_day_women",
   COUNT(CASE WHEN gender = 1 AND date_to_1000 <= reg_date + 3 			THEN external_id END) AS "1000_delay_3_day_reg_women"
