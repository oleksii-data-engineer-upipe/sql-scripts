...
threshold_dates AS (
   SELECT 
       ...
       MIN(spend_date) AS date_to_200
   FROM spend_data
   WHERE cumulative_spend >= 200
   GROUP BY 
       external_id, gender,
       first_purchase_date, reg_date,
       netw, site_id, os, country
...
SELECT 
   ...
   COUNT(CASE WHEN gender = 0 AND date_to_200 = first_purchase_date 		THEN external_id END) AS "200_day_to_day",
   COUNT(CASE WHEN gender = 0 AND date_to_200 = reg_date 				THEN external_id END) AS "200_day_to_day_reg",
   COUNT(CASE WHEN gender = 0  											THEN external_id END) AS "200_nakop",
   COUNT(CASE WHEN gender = 0 AND DATE_PART(month, date_to_200) = DATE_PART(month, reg_date) THEN external_id END) AS "200_nakop_reg",
   COUNT(CASE WHEN gender = 0 AND date_to_200 <= first_purchase_date + 3 THEN external_id END) AS "200_delay_3_day",
   COUNT(CASE WHEN gender = 0 AND date_to_200 <= reg_date + 3 			THEN external_id END) AS "200_delay_3_day_reg",

   -- Ті ж метрики для жінок
   COUNT(CASE WHEN gender = 1 AND date_to_200 = first_purchase_date 		THEN external_id END) AS "200_day_to_day_women",
   COUNT(CASE WHEN gender = 1 AND date_to_200 = reg_date 				THEN external_id END) AS "200_day_to_day_reg_women",
   COUNT(CASE WHEN gender = 1 											THEN external_id END) AS "200_nakop_women",
   COUNT(CASE WHEN gender = 1 AND DATE_PART(month, date_to_200) = DATE_PART(month, reg_date) THEN external_id END) AS "200_nakop_reg_women",
   COUNT(CASE WHEN gender = 1 AND date_to_200 <= first_purchase_date + 3 THEN external_id END) AS "200_delay_3_day_women",
   COUNT(CASE WHEN gender = 1 AND date_to_200 <= reg_date + 3 			THEN external_id END) AS "200_delay_3_day_reg_women"
